...
指定した列のユニーク値の組み合わせ（直積）と、暗黙の欠測を明示化
...
`expand()` 関数はと `complete()` 関数が出力するデータフレームの行数は、指定された列のユニーク値の数の積に比例して増加します。したがって、指定された列やそのユニーク値の数が多い場合、メモリ使用量が大きくなる可能性があります。

出力されるデータフレームの行数が1千万行を超えると推定された場合、以下のコードのような UserWarning が出力されます。

``` python
import pointblank as pb
data = pb.load_dataset("game_revenue")

res = py4st.expand(data, 'session_id', 'session_start', 'time')
#> UserWarning: expand() will generate 759,646,836 rows from the Cartesian product of selected columns. This may consume large memory and take significant time.
``` 
