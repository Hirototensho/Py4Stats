...
R言語の [`DescTools::Freq()`](https://cran.r-project.org/web/packages/DescTools/DescTools.pdf) 関数をオマージュした、度数分布表を計算する関数です。度数 `freq` と相対度数 `perc` に加えて、それぞれの累積値を計算します。
...
- `subset`：**str or list of str**（必須）</br>
　集計に使用するデータフレームの列名。
- `sort_by`：**str**</br>
...
![](https://img.shields.io/badge/lifecycle-deprecated-orange){fig-align="left" fig-alt="badge of lifecycle deprecated"}</br>
- `sort`：**Deprecated.**.</br>
　現在は `sort_by` の使用を推奨しています。この引数は後方互換性のために保持されおり、指定された場合は `FutureWarning`が発生します。デフォルトは `None` です。
...
#> 1  Chinstrap    68  0.197674      220  0.639535
#> 2     Gentoo   124  0.360465      344  1.000000

print(py4st.freq_table(penguins, ['island', 'species'], sort_by = 'frequency'))
...
[`py4stats.plot_count_h()`](./plot_count.qmd) 関数や [`py4stats.plot_count_v()`](./plot_count.qmd) 関数を使うと、`freq_table()` 関数で算出した度数分布を棒グラフとして可視化することができます。

## 注意 Notes

- Py4Stats v0.6.0 以降: R言語の [`DescTools::Freq()`](https://cran.r-project.org/web/packages/DescTools/DescTools.pdf) 関数の挙動との一貫性を確保するため、度数分布表をソートする基準を制御する `sort_by` 引数の初期設定を度数 `sort_by = 'frequency'` から `subset` に指定された変数の値 `sort_by = 'values'` に変更しました。
