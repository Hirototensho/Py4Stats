...
z.extend([2, 3])
build.assert_numeric(z, arg_name = 'z', lower = 0, upper = 1)
#> ValueError: Argument `x` must have value 0 <= x <= 1.
...
また、引数に None が指定されると初期設定ではエラーを出力しますが、`nullable = True` で `None` を許容することができ、`None` ます。`nullable = True` かつ `arg` に `None` が代入された場合、引数の型や値に関するアサーションは省略されます。
