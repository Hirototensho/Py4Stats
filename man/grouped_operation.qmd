...
- `func`：**callable**</br>
  ...
  - `group_map()`: `func` には任意の Python オブジェクトを返す関数を指定できます。回帰モデルの結果オブジェクトやグラフなど、表形式に変換できないオブジェクトを返す用途にも適しています。</br>
  - `group_modify()`: `func` の戻り値はデータフレームに変換可能なオブジェクトである必要があります。使用可能な戻り値の例は以下の通りです。</br>
    - データフレーム互換オブジェクト (`IntoFrameT`)</br>
    - Series 互換オブジェクト (`IntoSeriesT`)</br>
    - スカラー値（`int, float, bool, str`）</br>
    - `None`</br>
...
`group_modify()` と [`py4st.median_qi()`](./point_range.qmd) を組み合わせることで、グループ別に適用することができます。
...
`group_map()` と `smf.ols()`、そして [`py4st.compare_ols()`](./compare_ols.qmd) を組み合わせることで、次のように回帰分析の層別解析を実行することができます。
