...
データセットかれ除外された列が多い場合、レビューテキストの視認性を維持するために列名の一部が省略表記されます。
...
#> Columns removed: 'repwage', 'hushrs', 'husage', 'huseduc', 'huswage' and other 10 column(s)
#> 
#> No existing columns had their type changed.
#> 
...
列のデータ型に基づいた比較については [`compare_df_cols()`](./compare_df_cols.qmd) を、数値変数の統計値に基づいた比較については [`compare_df_stats()`](./compare_df_cols.qmd) もご参照ください。
