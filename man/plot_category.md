...
- `data`：**IntoFrameT**（必須）<br>
    カテゴリ変数を含む入力データフレーム（1列につき1変数）。 narwhals がサポートする任意のデータフレーム型（例：pandas.DataFrame、polars.DataFrame）を指定できます。すべての列は、同一のカテゴリ体系（共通のカテゴリラベル）を共有している必要があります。
- `sort_by`: **str**<br>
...
- `ax`:<br>
    描画先となる matplotlib の Axes。`None` の場合は、新しい Figure と Axes が作成されます。デフォルトは `None` です。
