...
　`Py4Stats` では [`compare_ols()`](./compare_ols.qmd) 関数の `stats_glance` 引数のように、いくつかの選択肢から文字列で指定する引数のアサーションに `build.arg_match()` 関数を使用することで、適切な値の選択肢を提示できるようにしています。
...
fit1 = smf.ols('body_mass_g ~ bill_length_mm', data = penguins).fit()

compare_tab1 = py4st.compare_ols(
    list_models = [fit1],
    stats_glance = ['rsq', 'nobs', 'df']
    )
#> ValueError: `stats_glance` must be one of 'rsquared', 'rsquared_adj', 'nobs', 'df', 'sigma', 'F_values', 'p_values', 'AIC' or 'BIC', not 'rsq'.
#>              Did you mean 'rsquared' or 'rsquared_adj'?
```
***
