...
Original file is located at
    https://colab.research.google.com/drive/1PNLk1YnM1WBxeId_-fdc2b6JAKhRmy_8
...
　`reg.compare_logit()` は計量経済学の実証論文でよく用いられる、回帰分析の結果を縦方向に並べて比較する表をする関数です。
　使用方法は次の通りで、`smf.logit()` や `smf.probit()` で作成した分析結果のオブジェクトのリストを代入します。  


### 引数

　　`reg.compare_logit()` 関数に指定できる引数は次の通りです。
...
def compare_logit(
    ...
    **kwargs
    ...
    # 引数に妥当な値が指定されているかを検証
    ad = match_arg(ad, ['overall', 'mean', 'median', 'zero'])
    ...
    if method == 'coef':
        ...
        tidy_list = [
            mod.get_margeff(dummy = dummy, at = at, method = method).summary_frame()
