...
def plot_count_h(
    ...
    The frequency table is internally computed using `py4stats.freq_table()`.
...
def plot_count_v(
        ...
        padding: int = 3,
        digits: int = 1,
        expand_y_limits: Union[float, int] = 1.1,
        ax: Optional[Axes] = None,
        ):
    """Plot a vertical bar chart of frequency counts for a categorical variable.

    This function computes a frequency table for the specified categorical
    variable and visualizes it as a vertical bar chart. Both absolute
    frequency and relative frequency (percentage) are displayed as labels
    on each bar.
    ...
    Args:
        ...
        digits (int, optional):
            Number of decimal places for percentage labels.
            Defaults to 1.
        expand_y_limits (float, optional):
            Multiplicative factor used to expand the upper limit of the
            y-axis to ensure sufficient space for labels.
            Defaults to 1.5.
        ax (Optional[Axes], optional):
            Existing Matplotlib Axes object to draw the plot on.
            If None, a new figure and axes are created. Defaults to None.
    ...
    build.assert_count(padding, arg_name = 'padding', len_arg = 1)
    build.assert_count(digits, arg_name = 'digits', len_arg = 1)
    build.assert_numeric(expand_y_limits, arg_name = 'expand_y_limits', len_arg = 1)
    # =========================================================
    ...
    bars = ax.bar(count_table[valiable], count_table['freq'], color = color)
    
    # ここでラベルを作図
    ax.bar_label(
        bars, 
        labels = count_table['label'], 
        padding = padding, 
        fontsize = fontsize
        );

    ax.set_ylim(0, count_table['freq'].max() * expand_y_limits)
