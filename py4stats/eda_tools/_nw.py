...
def draw_ascii_boxplot(data, width=40):
    ...
    median = data.quantile(0.25, 'midpoint')
    ...
    def scale(val):
        return int((val - min_val) / data_range * (width - 1))
...
def range_graph(before, after, col, spl = 7, spr = 7, width=40, digits = 2):
    ...
    boxplot_before = draw_ascii_boxplot(before[col], width)
    boxplot_after = draw_ascii_boxplot(after[col], width)
...
def review_numeric(before, after, digits = 2, width = 30):
    before_nw = remove_empty(before, to_native = False)\
        .pipe(remove_constant, to_native = False)\
        .select(ncs.numeric())
    ...
    review = ['Numeric value ranges (for reference):'] + review
