...
Original file is located at
    https://colab.research.google.com/drive/1UfglPXkd1rOY3KBg_yQUCFBxK6_OCDVS
...
def freq_table(data, x, sort = True, group = None, dropna = False):
    ...
    if group is None:
        ...
        res = res.rename(columns = {'value':'度数'})

        if sort: res = res.sort_values([group, '度数'], ascending = [True, False])

        res['総数'] = res.groupby(group).transform('sum')
        res['相対度数'] = res['度数'] / res['総数']
        res['累積度数'] = res.groupby(group)['度数'].cumsum()
        res['累積相対度数'] = res.groupby(group)['相対度数'].cumsum()
        res = res.drop('総数', axis = 'columns')
...
def make_rank_table(data, group, values, aggfunc = 'sum'):
    ...
    # シェア率と累積相対度数を計算
    rank_table['シェア率'] = (rank_table[values] / rank_table[values].sum())
    rank_table['累積シェア率'] = rank_table['シェア率'].cumsum()
    return rank_table
    # return p_table
...
def Pareto_plot(
    ...
    # グラフの見やすさのために上位 top_n 件を抽出します。
    if values is None:
        ax.bar(shere_rank.index, shere_rank['度数'], color = palette['bar'])
        ax.set_ylabel('度数', fontsize = fontsize * 1.1)
    else:
        # yで指定された変数の棒グラフ
