...
- [🚀 **Get Started**](./introduction.qmd)
- [📖 **API Reference**](./reference.qmd)

本ライブラリの主な機能は [Get Started](./introduction.qmd) を、実装されている関数の一覧は [Function reference](./reference.qmd) を参照してください。
...
[`py4stats.diagnose()`](man/diagnose.qmd) 関数はデータの全般的な状態についての要約を提供します。
...
[`py4stats.compare_ols()`](man/compare_ols.qmd) 関数は、計量経済学の実証論文でよく用いられる、回帰分析の結果を列方向に並べて比較する表を作成します。
...
詳細は、[`py4stats.compare_ols()`](man/compare_ols.qmd) を参照してください。　
